...
def main():
    filename = "C:/Users/nsuri/Desktop/IndependentStudy/Data/ProcessWikiDataForTopicModeling/TopicModelInput.txt";
    WikiKeywordsSplitsResult = "C:/Users/nsuri/Desktop/IndependentStudy/Data/Category/Processed_Data/TopicModelingResults.txt"
    output_file_categories=open(WikiKeywordsSplitsResult, "a")
    #filename = "C:/Users/nsuri/Desktop/MalletInputDir/data/file4.txt";
    parser = OptionParser()
    ...
    labelset, corpus, labels = load_corpus(filename)
    ...
    line = 'M= {} , V= {} , L= {} ,  K= {} '.format(len(corpus), len(llda.vocas), len(labelset), options.K) 
    print(line, file=output_file_categories)
   
    
    for i in range(options.iteration):
        sys.stderr.write("-- %d : %.4f\n" % (i, llda.perplexity()))
        line = "-- {} : {} \n".format(i, llda.perplexity()) 
        print(line, file=output_file_categories)
        llda.inference()
    
    print("perplexity : %.4f" % llda.perplexity())    
    line = 'perplexity :  {}'.format(llda.perplexity() ) 
    print(line, file=output_file_categories)
    ...
    #for i in range(options.iteration):
        ...
        #llda.inference()
    #print llda.z_m_n # author commented
    ...
    for v, voca in enumerate(llda.vocas):
        ...
        line = '{}'.format(','.join([voca]+[str(x) for x in phi[:,v]]) ) 
        print(line, file=output_file_categories)
    ...
    for k, label in enumerate(labelset):
        ...
        for w in numpy.argsort(-phi[k])[:20]:
            ...
            line = '{} : {}'.format(llda.vocas[w], phi[k,w]) 
            print(line, file=output_file_categories)
