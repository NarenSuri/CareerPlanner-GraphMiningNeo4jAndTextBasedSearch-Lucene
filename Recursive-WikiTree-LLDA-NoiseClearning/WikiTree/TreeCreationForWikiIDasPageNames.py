...
Level1={}
levelVal = [2,5,7,10,13,15,18,20] # for now hardcoding 8 levels to save. we can automate this
...
def write_report(r, filename):
    global writingFailed
    print("Writing to the file")
    with codecs.open(filename, 'a', 'ascii') as input_file:
        ...
        for k, v in r.items():
            if len(set(v)) >= minLengthOfCategories:
                ...
                try:
                    categories = ''.join([i if ord(i) < 128 else ' ' for i in categories])                    
                    k = ''.join([i if ord(i) < 128 else ' ' for i in k])  
                    '''
                    decoded_str_category = categories.decode("windows-1252")
                    encoded_str_category = decoded_str_category.encode('ascii', 'ignore')
                    decoded_str_k = k.decode("windows-1252")
                    encoded_str_k = decoded_str_k.encode('ascii', 'ignore')
                    '''                    
                    line = '{}, {}'.format(k, categories)
                    print(line, file=input_file) 
                except UnicodeEncodeError:
                    print(line)
                    writingFailed = writingFailed+1
                    continue
            else:
                continue
        input_file.close()
        print("Writing finished")    
        print("The total number of Writings failed cummulative unitl now are : " + str(writingFailed))  
    ...
    if CurrentNode.isCategory==1: 
     ...
     #************     
        ...
        #************ 
                        ...
                        DontHaveLevelsYouAskedFor = DontHaveLevelsYouAskedFor+1
                ...
                if line!="":
                    ...
                    if i==0:
                        try:
                            ...
                            print ("Probelm with this line : " + str(linenumberOfArticles))
                            breaking=1
                            break
                    
                    elif i==1:
                        #value= unidecode(line).lower().translate(replace_punctuation)
                        value= line.encode('utf-8').lower().translate(replace_punctuation)
                        breaking=0
                        ...
                        categoriesList=line.encode('utf-8').lower().strip().split("|")
...
def startWikiWordsExtractions():
    ...
    write_report(Level1, WikiKeywordsSplitsResult+str(st)+"_Result_Traversal_ForTopicModeling_Level"+str(levelVal[0])+".csv")
    write_report(Level2, WikiKeywordsSplitsResult+str(st)+"_Result_Traversal_ForTopicModeling_Level"+str(levelVal[1])+".csv")
    write_report(Level3, WikiKeywordsSplitsResult+str(st)+"_Result_Traversal_ForTopicModeling_Level"+str(levelVal[2])+".csv")
    write_report(Level4, WikiKeywordsSplitsResult+str(st)+"_Result_Traversal_ForTopicModeling_Level"+str(levelVal[3])+".csv")
    write_report(Level5, WikiKeywordsSplitsResult+str(st)+"_Result_Traversal_ForTopicModeling_Level"+str(levelVal[4])+".csv")
    write_report(Level6, WikiKeywordsSplitsResult+str(st)+"_Result_Traversal_ForTopicModeling_Level"+str(levelVal[5])+".csv")
    write_report(Level7, WikiKeywordsSplitsResult+str(st)+"_Result_Traversal_ForTopicModeling_Level"+str(levelVal[6])+".csv")
    write_report(Level8, WikiKeywordsSplitsResult+str(st)+"_Result_Traversal_ForTopicModeling_Level"+str(levelVal[7])+".csv")
    ##############################################################
        # Do random Sampling
